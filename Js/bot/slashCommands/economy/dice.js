...
module.exports = {
    ...
    run: async (bot, interaction, args) => {    
        ...
        .setColor("RED")
        .setDescription(`❌ <@${member.id}> : You have  \`PASSIVE\` activated, you need to desactivate it.`);
        ...
        if (botRoll < userChoice) {
            ...
            .setThumbnail(member.user.displayAvatarURL({ format: 'png', size: 256, dynamic: true }))
            .setFooter(`Asked by ${member.nickname} • ${interaction.guild.name}`,interaction.guild.iconURL())
            .setDescription(`Dice Beta V1 | Player **${member.nickname}** \n\n${bot.member.nickname} rolled: \`${botRoll}\` \n${member.nickname} rolled: \`${userChoice}\`\n\n**${member.nickname}** & **${bot.member.nickname}**: Draw\n\nLost: **${tieCoins.toLocaleString()}** :coin:`)
