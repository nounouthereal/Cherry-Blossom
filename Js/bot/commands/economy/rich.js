...
let data = await economy.find().sort([['coinsInWallet', 'descending']])
    ...
    if (data.length == 0) return message.channel.send(':warning: Personne n\'est riche ici'); 
    
    const emojis = [':first_place:', ':second_place:', ':third_place:'];
    data = data.map((x, i) => `${emojis[i] || '🔹'} **${x.coinsInWallet.toLocaleString()} :coin:** - ${bot.users.cache.get(x.userId).tag || 'Unkown#0000'}`);

    const embed = new MessageEmbed()
        .setAuthor(`🤑 Personne les plus riches de ${message.guild.name}`)
...
module.exports.config = {
    ...
    aliases: ['leader','lbmoney','leaderbord','topmoney'], // Aliases 
