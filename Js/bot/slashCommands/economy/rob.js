    ...
    run: async (bot, interaction, args) => {
        ...
        if (robbedUser.coinsInWallet < 1000) {
            let rob4embed = new MessageEmbed()
                .setColor("BLUE")
                .setDescription(`🛡 <@${usertag.user.id}> : The user you tried to steal has a protection on his balance at the moment because he has a balance of less than \`1 000\` :coin:.`);
        ...
        if (user.coinsInWallet < 1000) {
            let rob4embed = new MessageEmbed()
                .setColor("BLUE")
                .setDescription(`🛡 <@${usertag.user.id}> : You must have at least balance of \`1 000\` :coin: to rob someone.`);
            return interaction.followUp({ embeds: [rob4embed] });
            //return interaction.followUp("This user doesn't have much coins, I wouldn't rob them");
        }
