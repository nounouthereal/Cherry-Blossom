...
client.on("message", async message => {
    ...
    if(!message.content.startsWith('+')&& message.content.startsWith(client.user.id)) return message.reply(`My Prefix is: **\`+\`**, type \`+help\` for more information!`); //if the messages is not a command and someone tags the bot, then send an info msg
    if (!message.content.startsWith('+')) return; //if the message does not starts with the prefix, return, so only commands are fired!
    
    const args = message.content.slice('+'.length).trim().split(/ +/g); //creating the argumest (each space == 1 arg)
    ...
    if (command) //if the command is now valid
    {
      ...
      try{
        command.run(client, message, args, message.author, args.join(" "), '+'); //run the command with the parameters:  client, message, args, user, text, prefix, 
        /* /////////////////////////////////////////
        HERE AN EXAMPLE:
    ...
    return message.reply(`Unkown command, try: **\`+fhelp\`**`)
