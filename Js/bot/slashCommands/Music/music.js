    ...
    run: async (bot, interaction, args) => {
        ...
        try {
            ...
            if (args[0] == "play") {
                ...
                if (!res || !res.tracks.length) {
                    let errEmbed = new MessageEmbed()
                        ...
                        .setColor("RED")
                    return interaction.editReply({embeds: [playing]}) || interaction.followUp({ embeds: [errEmbed] })
                }
                ...
                if (!channel) {
                    let noChannelEmb = new MessageEmbed()
                        ...
                        .setColor("RED")
                    return interaction.editReply({embeds: [playing]}) || interaction.followUp({ embeds: [noChannelEmb] })
                }
                    ...
                    if (!channel.viewable) {
                        let cannotSeeChannelEmb = new MessageEmbed()
                            ...
                            .setColor("RED")
                        return interaction.editReply({embeds: [playing]}) || interaction.followUp({ embeds: [cannotSeeChannelEmb] })
                    }
                    ...
                    if (!channel.joinable) {
                        let perm1ChannelEmb = new MessageEmbed()
                            ...
                            .setColor("RED")
                        return interaction.editReply({embeds: [playing]}) || interaction.followUp({ embeds: [perm1ChannelEmb] })
                    }
                    ...
                    if (!channel.speakable) {
                        let perm2ChannelEmb = new MessageEmbed()
                            ...
                            .setColor("RED")
                        return interaction.editReply({embeds: [playing]}) || interaction.followUp({ embeds: [perm2ChannelEmb] })
                    }
                    ...
                    if (channel.full) {
                        let fullChannelEmb = new MessageEmbed()
                            ...
                            .setColor("RED")
                        return interaction.editReply({embeds: [playing]}) || interaction.followUp({ embeds: [fullChannelEmb] })
                    }
                }
                ...
                if (queue.playing) {
                    let playing = new MessageEmbed()
                        .setDescription(`❌ <@${interaction.user.id}> : Seems like we are already playing [${queue.current.title}](${queue.current.url}) in <#${queue.metadata.id}>. **Try** \`/music playnext\` **command to add a song in your queue**.`)
                        .setColor("RED")
                    return interaction.editReply({embeds: [playing]}) || interaction.followUp({ embeds: [playing] })
                }
                ...
                try {
                    ...
                    let joinErr = new MessageEmbed()
                        ...
                        .setColor("RED")
                    return interaction.editReply({embeds: [playing]}) ||  interaction.followUp({ embeds: [joinErr] })
                }
            ...
            if (args[0] == "playnext") {
                ...
                if (!queue?.playing) {
                    let playing = new MessageEmbed()
                        .setDescription(`❌ <@${interaction.user.id}> : Seems like no song is played. **Try** \`/music play\` **command to play a song.**`)
                        .setColor("RED")
                    return interaction.editReply({embeds: [playing]}) || interaction.followUp({ embeds: [playing] })
                }
                ...
                if (queue?.playlist) {
                    let playing = new MessageEmbed()
                        .setDescription(`❌ <@${interaction.user.id}> : This command does not support playlist. *Try* \`/music play\` *command to play a playlis.*.`)
                        .setColor("RED")
                    return interaction.editReply({embeds: [playing]}) || interaction.followUp({ embeds: [playing] })
                }
